...
const FileUploadModal = (props) => {
  ...
  return (
    <Modal isOpen={modal} toggle={toggle} centered>
      ...
      <ModalBody>
        <StyledQuillEditor className={"chat-input"} modules={modules} ref={refCallback} placeholder={"Add message. Type @ to mention someone."} readOnly={uploading} onChange={handleQuillChange} />
        <FilesPreview files={files} onRemoveFile={handleRemoveFile} />
      </ModalBody>
      <ModalFooter>
        <Button color="primary" onClick={handleUpload}>
          {uploading && <span className="spinner-border spinner-border-sm mr-2" role="status" aria-hidden="true"></span>}
          Upload
        </Button>{" "}
            ...
            <li key={i}>
              ...
              {file.type !== "IMAGE" && (
                <DocDiv className="card app-file-list">
                  ...
                  <div class="p-2 small">{file.name}</div>
                </DocDiv>
              )}
            </li>
          );
