...
const UnfurlPrio = (props) => {
  ...
  return (
    <span className="unfurl-priority">
                ...
                <div className={"media-header"}>
                  {!fromDriff && (
                    <p>
                      ...
                      <a className={"provider-name"} href={item.data.provider_url} title={item.data.provider_name} target="_blank" rel="noopener noreferrer">
                        {item.data.provider_name}
                      </a>
                      {item.data.author_name ? <span className="item-author-name"> | {item.data.author_name}</span> : ""}
                    </p>
                  )}
                </div>
                <div className="media-body">
                  <h6 className={"item-title"}>
                    <a href={item.data.url} title={item.data.title} target="_blank" rel="noopener noreferrer">
                      {item.data.title}
                    </a>
                  </h6>
                  <div className={"description"} dangerouslySetInnerHTML={{ __html: item.data.description }} />
                </div>
                {item.data.task_status && (
                  <div className="unfurl-task-data">
                    <span className="unfurl-status">
                      <strong>Status</strong> -- {item.data.task_status}{" "}
                    </span>
                    <UnfurlPrio prio={item.data.task_priority} />
                    <br />
                    {item.data.task_participants && item.data.task_participants.length ? (
                      <span className="unfurl-assignees">
                        <strong>Assignees</strong> -- {item.data.task_participants.map((p) => p.name).join(", ")}
                      </span>
                    ) : null}
                  </div>
                )}
