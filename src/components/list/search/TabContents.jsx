    ...
    return (
        <div className="tab-content" id="myTabContent">
            ...
            <div className={`tab-pane fade ${(activeTab === null || activeTab === "people") && "active show"}`} id="users" role="tabpanel">
                { tabs.hasOwnProperty("PEOPLE") && <PeopleTabResults people={tabs.PEOPLE.items} page={tabs.PEOPLE.page}/> }
            </div>
            <div className={`tab-pane fade ${(activeTab === null || activeTab === "post") && "active show"}`} id="photos" role="tabpanel">
                { tabs.hasOwnProperty("POST") && <PostsTabResults posts={tabs.POST.items} page={tabs.POST.page}/> }
            </div>
            <div className={`tab-pane fade ${(activeTab === null || activeTab === "workspace") &&  "active show"}`} role="tabpanel">
                { tabs.hasOwnProperty("WORKSPACE") && <WorkspaceTabResults workspaces={tabs.WORKSPACE.items} page={tabs.WORKSPACE.page}/> }
            </div>
        </div>
    );
};
