...
const BackIcon = styled(SvgIconFeather)`cursor: pointer; `;
const ResultWrapper = styled.ul`list-style:none;background:transparent;margin:0;padding:0;li:last-child{border-bottom:none}`;
const ResultItem = styled.li`cursor:pointer;background:transparent;font-size:0.835re;padding:5px 20px;margin-top:0;border-radius:0;border-left:none;border-right:none;border-top:none;border-bottom:1px solid #ebebeb;.dark &{border-color:rgba(155,155,155,.1)}color:#495057;p.chat-search-date{font-size:smaller;margin:0}div.chat-search-body{font-size:0.835re}div.chat-search-body span{font-weight:600;color:#7A1B8B}`;
const ResultDivWrapper = styled.div`overflow: auto;height: calc(100% - 43px);`;
const EmptyState = styled.div`display:flex;flex-flow:column;justify-content:center;align-items:center;flex-grow:1;padding:5rem;svg{display:block;margin:0 auto}h3{font-size:16px}h5{margin-bottom:0;font-size:14px}button{width:auto!important;margin:2rem auto}color:#363636;`;
...
const ChatSearchPanel = (props) => {
  ...
  const clear = () => {
    setResults([]); setQuery(""); setSearching(false);
  };
  ...
  useEffect(() => {
    ...
    } else { setResults([]); inputGroup.current.classList.remove("not-empty"); }
  }, [query])
