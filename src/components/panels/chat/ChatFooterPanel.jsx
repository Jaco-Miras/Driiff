...
const ChatFooterPanel = (props) => {
  ...
  const handleStartGoogleMeet = () => {
    ...
    )}" rel="noopener noreferrer" target="_blank"><strong>Click here to join</strong></a></div><div>Notice this meeting only works if you and the participant are using the same Google WorkSpace domain. If you both use a different domain, share the Google link generated by Google.</div>`;
    let payload = {
      channel_id: selectedChannel.id,
      body: messageBody,
      mention_ids: [],
