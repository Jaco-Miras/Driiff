        ...
        <PostContent onClick={() => openPost(post)}>
          ...
          <div className="d-flex align-items-center justify-content-between flex-grow-1 min-width-0 mr-1">
            <div className={`app-list-title text-truncate ${isUnread ? "has-unread" : ""}`}>
              ...
              <PostReplyCounter>
                {post.author.id !== userId && post.unread_count === 0 && post.view_user_ids && !post.view_user_ids.some((id) => id === userId) && <div className="mr-2 badge badge-secondary text-white text-9">{dictionary.new}</div>}
