...
const MainSnooze = (props) => {
  ...
  const dictionary = {
    ...
    snoozeAll: _t("SNOOZE.SNOOZE_ALL", `Snoozed for ${snoozeTime} min(s)`), snoozeMe: _t("SNOOZE.SNOOZE_ME", `Snoozed for ${snoozeTime} min(s)`),
  ...
  const processItems = (type, items) => {
    ...
    items.map((n) => {
      ...
      if (type === 'notification') {
        if (n.type === "POST_MENTION")
          !n.is_read && !n.is_snooze ? snooze.push(data) : n.is_read && toast.isActive(elemId) && toast.dismiss(elemId);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (Object.keys(chats.channels).length > 0 && users.users) {
        ...
        const todos = unSnoozeMe('todo', todoCLean()), notis = unSnoozeMe('notification', notifCLean()), huddles = unSnoozeMe('huddle', huddleBots);
