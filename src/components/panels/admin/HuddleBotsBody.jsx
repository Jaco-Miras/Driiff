...
import { setFilter } from "../../../redux/actions/adminActions";
...
const HuddleBotsBody = (props) => {
  ...
  const dictionary = {
    ...
    huddleLabel: _t("ADMIN.HUDDLE_LABEL", "Huddle bot"),
  };
  ...
  useEffect(() => {
    dispatch(setFilter({ filters: { ...filters, bots: true } }));
    if (!loaded) {
      //get the bots
      let cb = (err, res) => {
        if (err) return;
    ...
    <Wrapper>
      {huddleBot && (
        <div className="card-body">
          <h4 className="mb-3">{dictionary.huddleLabel}</h4>
          <div className="mb-2">
          ...
          <div className="mb-2">
            ...
            <div className="mb-2">
              <div className="input-group">
                <input onChange={handleMessageChange} data-name={"introduction_message"} type="text" className="form-control" placeholder={"Welcome message"} aria-describedby="button-addon1" autoFocus value={form.introduction_message} />
              </div>
            </div>
          </div>
          <div className="mb-2">
