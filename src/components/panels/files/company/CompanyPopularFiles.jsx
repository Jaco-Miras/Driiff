  ...
  return (
    <Wrapper className={`popular-files ${className}`}>
      ...
      <div className="row">
        {files.length > 0 &&
          files.map((f) => {
            return <CompanyFileListItem key={f.id} className="col-xl-3 col-lg-4 col-md-6 col-sm-12" file={f} actions={actions} folders={folders} files={files} disableOptions={disableOptions} />;
          })}
      </div>
    </Wrapper>
  );
...
export default React.memo(CompanyPopularFiles);
