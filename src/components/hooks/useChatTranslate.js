...
const useChatTranslate = (props) => {
  const { message, isAuthor, translate, chat_language, actions, isTranslateChannels } = props;
  ...
  useEffect(() => {
    ...
    if (!isAuthor && message.user.chat_language !== chat_language && translate && !message.is_translated && message.translated_language !== chat_language && !isTranslateChannels) {
