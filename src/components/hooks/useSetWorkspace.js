...
const useSetWorkspace = () => {
    ...
    const getAndSetChannel = useCallback(
        (code) => {
            if (!fetchingChannel) {
    ...
    useEffect(() => {
        ...
        if (workspacesLoaded && activeTopic === null && activeTopicSettings !== null) {
            ...
            if (topic) {
                if (typeof topic.topic_detail === "undefined") {
                    topic = {
                        ...topic,
                        topic_detail: {
                            active: topic.active,
                ...
                dispatch(setActiveTab(topic.is_external === 0 ? "intern" : "extern"));
                dispatch(setActiveTopic(topic));
            }
        }
        ...
        if (activeTopic && match.url === "/workspace/chat") {
            ...
            if (params.workspaceId !== undefined && parseInt(params.workspaceId) !== activeTopic.id) {
                ...
                if (params.hasOwnProperty("folderId")) {
                    ...
                    workspace.topic_detail = {};
                    workspace.topic_detail.active = workspace.active;
                    workspace.topic_detail.channel = workspace.channel;
