...
class Socket extends PureComponent {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (this.props.unreadChatReplies !== prevProps.unreadChatReplies || this.props.notificationLength !== prevProps.notificationLength) {
    ...
    if (
      prevProps.unreadPostEntries !== this.props.unreadPostEntries ||
  ...
  onListen = (external = localStorage.getItem("slug"), ex_id = this.props.user.id) => {
    ...
    // audio.src = chatSound;
    window.Echo.channel(external + ".App.User.Inactive").listen(".user-inactive", (e) => {
    ...
    window.Echo.channel(external + ".App.roles").listen(".updated-roles", (e) => {
  ...
  render() {
    return <PreloadEmojiImage src={"https://unpkg.com/emoji-datasource-apple@5.0.1/img/apple/sheets-256/64.png"} />;
  }
}

function mapStateToProps({ session: { user }, settings: { userSettings }, chat: { channels, selectedChannel }, posts: { posts }, workspaces: { activeTopic, workspaceChannelIds } }) {
  return {
    user,
